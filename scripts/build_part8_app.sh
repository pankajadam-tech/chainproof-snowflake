...
if [[ "$RESET_APP" == "1" ]]; then
  echo "=== Part 8 controlled APP reset ==="
...
echo "=== Create or replace Part 8 read-only APP views and stage ==="
...
echo "=== Run readable Part 8 Snowflake validation ==="
snow sql "${SNOW_SQL_OPTS[@]}" -f snowflake/51_part8_app_validation.sql

echo "=== Run fail-fast Part 8 Snowflake tests ==="
...
echo "=== PART 8 BUILD PASS ==="
echo "Six read-only APP views and the ChainProof Streamlit object passed deterministic validation."
