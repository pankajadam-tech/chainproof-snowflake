...
from .analyst_core import ANALYST_ENDPOINT, build_request, parse_response, validate_read_only_sql
...
def send_analyst_message(messages: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if status >= 400:
        message = payload.get("message", response.get("reason", "Unknown Analyst error")) if isinstance(payload, dict) else str(payload)
...
def execute_analyst_sql(session: Any, sql: str):
